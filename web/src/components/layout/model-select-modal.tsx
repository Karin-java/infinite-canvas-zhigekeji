...
export function ModelSelectModal({ open, channel, selectedModels, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedModels: ChannelModel[]; onConfirm: (models: ChannelModel[]) => void; onClose: () => void }) {
    ...
    const visibleList = useMemo(() => {
        ...
        return keyword ? currentList.filter((model) => model.name.toLowerCase().includes(keyword)) : currentList;
    }, [currentList, search]);
    ...
    const confirm = () => {
        ...
        const ordered = [...existing, ...fetched].filter((model) => {
            if (seen.has(model.name) || !selected.has(model.name)) return false;
            seen.add(model.name);
            return true;
        });
        onConfirm(ordered);
