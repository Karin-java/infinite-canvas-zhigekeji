...
+ [新增] 提供 Windows 与 macOS 双击启动器，首次运行自动准备 Bun 和项目依赖并打开本地画布。
+ [调整] 知格科技视频调用脚本改为每 5 秒查询一次，最多等待 15 分钟。
+ [新增] 模型调用脚本新增知格科技图片、视频和文本模板，拉取该渠道模型时会读取 `name` 与 `type` 字段、按名称去重并自动填入对应脚本。
