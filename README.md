...
这个备份分支在原项目基础上增加了 [知格科技](https://zhigekeji.com/docs) API 的开箱即用适配：

- 模型列表兼容知格返回的 `name` 和 `type` 字段，自动识别生图、视频与文本能力，并合并同名模型。
- 生图使用知格的 JSON 请求格式；文本使用 `/chat/completions`。
- 视频使用异步任务接口，每 5 秒查询一次状态，最长等待 15 分钟，适配耗时较长的视频生成。
- 模型调用脚本提供“知格科技”模板；使用知格接口地址拉取模型时，也会自动绑定对应脚本。

### 使用方法

1. 启动画布，打开右上角“配置”，进入“渠道”，点击“新增渠道”。
2. 协议选择 `OpenAI`，接口地址填写 `https://zhigekeji.com/api/v1`，API Key 填写你自己的知格密钥。
3. 点击“选择模型” → “拉取模型列表”，勾选需要的生图、视频或文本模型后确认，再保存渠道。
4. 回到画布，在相应节点选择刚添加的模型即可生成。视频任务可能需要约 15 分钟，请保持页面打开并等待完成。

如果模型列表中出现同名模型，画布会保留第一条；如上游标注的能力不符合实际，可在渠道模型列表中手动切换“生图 / 视频 / 文本”。不要把真实 API Key 提交到 Git 仓库或分享给他人。
